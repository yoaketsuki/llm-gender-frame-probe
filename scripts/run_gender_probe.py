"""
Gender-issue probe -- 6 models x fixed 3-turn script x N repeats.
Condition: BARE (no system prompt), default temperature, thinking/reasoning ON & saved.
ASCII-only prints (cp950 console). File IO is utf-8.

Usage:
  python run_gender_probe.py             # all repeats 1..REPEATS, all models (resumes; skips existing)
  python run_gender_probe.py 1           # only repeat 1 (smoke: 6 models x 3 turns)
  python run_gender_probe.py 2 3         # repeats 2..3
  python run_gender_probe.py 1 1 Opus4.6 # repeat 1, only the Opus4.6 family
"""
...
LOG = BASE / "outputs" / "run_log.jsonl"

# ---- three fixed rounds (VERBATIM, single source of truth) ----
R1 = (
...
# ============================================================
# per-provider converse: multi-turn, capture (text, thinking)
# history carries ONLY text (thinking saved but not re-sent ->
# safest + most natural; a real user never sees prior thinking)
# ============================================================
...
def converse_gemini(model, rounds):
    ...
    for q in rounds:
        ...
        out.append({"text": text, "thinking": thinking,
...
# ============================================================
def save_sequence(family, model_id, rep, turns):
    fpath = RAW / ("%s_run%d.md" % (family, rep))
...
def run(lo, hi, fams):
    print("=== Gender probe rep %d..%d  fams=%s ===" % (lo, hi, sorted(fams) if fams else "ALL"))
    g_in = g_out = 0
    for spec in MODELS:
        if fams and spec["family"] not in fams:
            continue
        for rep in range(lo, hi + 1):
            fpath = RAW / ("%s_run%d.md" % (spec["family"], rep))
            if fpath.exists():
                print("SKIP exists: %s" % fpath.name)
                continue
            tag = "%s run%d [%s]" % (spec["family"], rep, spec["model"])
            print(">> %s ..." % tag, flush=True)
            rec = {"ts": TS, "family": spec["family"], "model": spec["model"], "rep": rep}
            try:
                turns = DISPATCH[spec["provider"]](spec["model"], ROUNDS)
                fp, tin, tout = save_sequence(spec["family"], spec["model"], rep, turns)
                g_in += tin; g_out += tout
...
if __name__ == "__main__":
    ...
    fams = set(a for a in sys.argv[1:] if not a.isdigit())
    if len(nums) == 0:
        lo, hi = 1, REPEATS
    elif len(nums) == 1:
        lo = hi = nums[0]
